...
//   ∘(p - round(∘(p / b)) * b) = ∘(p - 0 * b) = ∘(p) = p.
  ...
  bool RemainderCheckAllSolved(T p, T b) {
    ...
    T cmpmask = cmp_p<_CMP_GT_OQ>(abs_p_mul_2, b);
    return !testz_p(cmpmask, cmpmask);
  }
#else
  template<typename T>
  bool RemainderCheckAllSolved(T p, T b) {
    T abs_p_mul_2 = set1_p<T>(2.0) * Abs(p);
    T cmpmask = cmp_p<_CMP_GT_OQ>(abs_p_mul_2, b);
    return !movemask_p(cmpmask, cmpmask);
  }
#endif
...
template<typename T>
bool RemainderCheckAllFinite(T q) {
  // finite_q - finite_q = +0, ±inf - ±inf = nan, nan - nan = nan
  T hopefully_zero = q - q;
  return testz_si(cast_si(hopefully_zero), cast_si(hopefully_zero));
}
...
T RemainderReduction(T p, T b) {
  ...
  if (distopia_unlikely(!RemainderCheckAllFinite(q))) {
    ...
    // Trick: p mod b = (p mod (b * 2^m)) mod b for integer m ≥ 0.
    T scaled_b = b * set1_p<T>(0x1.p127);
  ...
  while (true) {
    {
      T qi = round_p<_MM_ROUND_NEAREST | _MM_FROUND_NO_EXC>(q);
