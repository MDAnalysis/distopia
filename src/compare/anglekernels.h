...
void angle_mic(const float *xyz, const int *triplets, const float *box_matrix,
               float *out, const int n_frames, const int n_atoms,
               const int n_angles)
#endif
#else
...
#endif
{
  ...
  for (int i = 0; i < n_angles; i++) {
    int pairs[4] = {triplets[3 * i + 1], triplets[3 * i], triplets[3 * i + 1],
                    triplets[3 * i + 2]};
#ifdef COMPILE_WITH_PERIODIC_BOUNDARY_CONDITIONS
#ifdef COMPILE_WITH_TRICLINIC
    dist_mic_triclinic(xyz, pairs, box_matrix, &distances[0], &displacements[0],
                       n_frames, n_atoms, 2);
#else
    _dist_mic_original(xyz, pairs, box_matrix, &distances[0], &displacements[0], n_frames, n_atoms, 2);

#endif
#else
    _dist_original(xyz, pairs, &distances[0], &displacements[0], n_frames, n_atoms, 2);
#endif
